...
4. --query:  the query protein sequences
5. --len:  Non-OR genes
6. --uniport: uniprot database
7. --quenm: the uniprot names of query names
                                                    Kang 2022-8-23
------------------------------------------------------------------------------------------
_EOH_
;
...
sub tblastn {
    ...
    system("solar.pl -f m8 -cCn 10000 -d -1 1_query.bla >2_bla.solar");
...
#        }
    ...
    open QUENM, $quenm or die "can not open $quenm\n";
    my %hashque;
    while (<QUENM>) {
        chomp;
        $hashque{$_}++;
    ...
    while (<POTEN>) {
        ...
        $cov=sprintf("%.3f",$cov);
        if ($hashque{$ano} && $iden>=40 && $cov>=0.7 && $evalue<=10e-20) {
