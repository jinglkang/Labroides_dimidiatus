# Gene family size detection based on orthofinder   
```bash
...
```
***   
### divid the orthogroups into small orthogroups   
```bash
...
```
***   
## No care about gene families including genes with different name
**1. Resulted Orthogroups as the gene family**   
```bash
...
```
**2. Estimate the F-index of the 13862 conserved gene families**  
```bash
...
# [1] 8707
```
**4. Estimate the calibrated tree**   
```bash
...
```
### 1. concatenate the cds sequences of single copy genes (after align and trim)
```perl1
#!/usr/bin/perl -w
use strict;
use warnings;
