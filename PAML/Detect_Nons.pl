...
my @nocl1=qw(Cund Zebrafish); # compare to "Node22"
...
sub Print_Nons {
        ...
        my $nb=keys %hash3;
        if ( $nb==1 && ($hash1{'Tbif'}->{$pos} ne $hash1{'Ncel'}->{$pos}) && ($hash1{'Smel'}->{$pos} ne $hash1{'Cund'}->{$pos}) && ($hash1{'Smel'}->{$pos} ne $hash1{'Zebrafish'}->{$pos}) ) {
                print "$orth\t$info\n";
        }
}
